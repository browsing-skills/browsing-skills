# X (Twitter) — Following Feed Reference

## Requirements

**Auth:** X requires login to view the home feed. Ask the user for their `auth_token` cookie from x.com (DevTools → Application → Cookies → `auth_token`). Inject before navigating:
...
**Note:** X's home feed is paginated by scroll — only posts visible in the current viewport (plus what has already been rendered above/below) are extracted. Scroll down before running the action if you want more results.
...
## Action: following-feed

Use when the user wants to see posts from their X home feed (either the "For You" algorithmic tab or the "Following" chronological tab).
...
```js
({
  name: "x-following-feed",
  description: "Extract visible posts from the X home feed (For You or Following tab)",
  ...
  execute: function(params) {
    ...
    data.feedTab = activeTab ? activeTab.textContent.trim() : "";
    ...
    if (mode === "display") {
      ...
      h += "<h2 style=\"color:#fff;margin:0 0 16px;\">Home Feed" + (data.feedTab ? ": " + esc(data.feedTab) : "") + " (" + data.totalResults + " posts)</h2>";
...
**Returns:** `{ feedTab, posts: [{ content, authorName, authorHandle, authorProfileUrl, verified, timestamp, postUrl, replies, reposts, likes, hasMedia, views }], totalResults }`
